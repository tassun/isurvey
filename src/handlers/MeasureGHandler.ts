...
export class MeasureGHandler extends SurveyOperateHandler {
    public readonly form_id : string = "MEASURE_G";
    public model : KnModel = {
        ...
        fields: {
            ...
            MG_1_4: { type: "STRING", created: true, updated: true  },
            MG_1_5: { type: "STRING", created: true, updated: true  },
            MG_C_mark: { type: "INTEGER", created: true, updated: true  },
            MG_C_total: { type: "INTEGER", created: true, updated: true  },
            MG_C_avg: { type: "DECIMAL", created: true, updated: true  },
    ...
    protected override processCalculate(context: KnContextInfo, db: KnDBConnector, data: KnDataSet) : KnDataSet {
        ...
        data.MG_C_total = 0;
        data.MG_C_avg = 0.0;
        let keys = Object.keys(data).filter(key => key.startsWith("MG_1_"));
        for (let key of keys) {
            let value = data[key];
        ...
        data.MG_C_total = keys.length;
        data.MG_C_avg = data.MG_C_total>0?data.MG_C_mark / data.MG_C_total:0.0;
        return data;
    }
