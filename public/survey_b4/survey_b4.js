...
function setupUI() {
    ...
    $('input[name="SB4_3_1"]').on('change', function() {
        if ($(this).val() == '2') {
...
function checkCaused() {
    ...
    let notnotify = $("#SB4_2_1_1").is(":checked");
    ...
    if(notsuccessed) {
        ...
        if(notnotify) {
            $("#SB4_2_4_4").attr('data-parsley-required', 'false');
        } else {
            $("#SB4_2_4_4").attr('data-parsley-required', 'true');
        }    
    }
    return false;
}
...
function gotoListingForm() {
    ...
    startWaiting();
    submitWindow({url: BASE_URL+"/survey_b4/listing", params: {token_key: token_key, profile_id: profile_id, survey_id: master_id, master_id: master_id, column_id: column_id}, windowName: "_self"});
}
