...
function setupUI() {
    ...
    $('input[name="SB7_3_1"]').on('change', function() {
        if ($(this).val() == '2') {
...
function enableRequiredFields(enabled="true") {
    ...
    $("#fault_character_1").attr('data-parsley-required', enabled);
    $("#fault_event_1").attr('data-parsley-required', enabled);
}
function checkCaused() {
    enableRequiredFields("true");
    let notnotify = $("#SB7_2_1_1").is(":checked");
    ...
    if(notsuccessed) {
        ...
        if(notnotify) {
            $("#SB7_2_4_4").attr('data-parsley-required', 'false');
        } else {
            $("#SB7_2_4_4").attr('data-parsley-required', 'true');
        }    
    }
    return false;
}
