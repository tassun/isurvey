...
function parseErrorThrown(xhr,status,errorThrown) {
	...
	try{
		...
		if(json.head.errorcode) {
			errorThrown = getMessageTitle(json.head.errorcode,errorThrown);
		}
	}catch(ex) { }
...
var smartmsgdoc = null;
function loadSmartMessage(aSync) {
	jQuery.ajax({
		url: "/assets/smart_message.json?seed="+Math.random(),
		async : aSync,
		type: "GET",
		contentType: "application/x-www-form-urlencoded; charset=UTF-8",
		dataType: "json",
		success: function(data){ smartmsgdoc = data; }
	});	
}
function getMessageCode(errcode, params) {
	try {
		if (smartmsgdoc == null) loadSmartMessage(false);
		let msgnode = smartmsgdoc[errcode];
		let fs_curlang = fs_default_language;
		if(!fs_curlang) fs_curlang = "EN";
		let fs_msgtext = msgnode?msgnode[fs_curlang.toUpperCase()]:undefined;
...
function getMessageTitle(titleCode, defaultTitle) {
	...
	if(!fs_msgtitle || fs_msgtitle=="" || fs_msgtitle==titleCode) fs_msgtitle = defaultTitle;
	return fs_msgtitle;
}
